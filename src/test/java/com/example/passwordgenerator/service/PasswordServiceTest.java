...
public class PasswordServiceTest {
    ...
    @Mock
    private BCryptPasswordEncoder passwordEncoder;  // Мок для BCryptPasswordEncoder
    ...
    public void setUp() {
        ...
        // Внедряем все моки, включая passwordEncoder, в PasswordService
        passwordService = new PasswordService(passwordRepository, passwordCache, passwordEncoder);
    }
    ...
    public void testCreate() {
        ...
        when(passwordEncoder.encode("pass1")).thenReturn("encodedPass1");  // Настраиваем мок
        ...
        assertEquals("encodedPass1", created.getPassword());  // Проверяем закодированный пароль
        assertEquals("user1", created.getOwner());
        verify(passwordEncoder).encode("pass1");  // Убеждаемся, что encode вызван
    }
    ...
    public void testUpdate() {
        ...
        when(passwordEncoder.encode("updatedPass")).thenReturn("encodedUpdatedPass");  // Настраиваем мок
        when(passwordRepository.save(any(Password.class))).thenReturn(password);
        Password updated = passwordService.update(password);
        assertEquals("encodedUpdatedPass", updated.getPassword());  // Проверяем закодированный пароль
        assertEquals("user1", updated.getOwner());
        verify(passwordEncoder).encode("updatedPass");  // Убеждаемся, что encode вызван
    }
